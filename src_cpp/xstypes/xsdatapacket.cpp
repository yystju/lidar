...
uint64_t XsDataPacket_sampleTime64(const XsDataPacket* thisPtr)
{
	...
	if (it != MAP.end())
		rv += it->second->toDerived<SimpleVariant<uint32_t>>().m_data % coarseFactor;
...
*/
void XsDataPacket_setRawGpsDop(XsDataPacket* thisPtr, const XsRawGpsDop* r)
{
	genericSet<XsRawGpsDop, XsRawGpsDopVariant>(thisPtr, r, XDI_GpsDop);
}
...
/*! \relates XsDataPacket
	...
	\param r The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsSol(XsDataPacket* thisPtr, const XsRawGpsSol* r)
{
	genericSet<XsRawGpsSol, XsRawGpsSolVariant>(thisPtr, r, XDI_GpsSol);
}
...
/*! \relates XsDataPacket
	...
	\param r The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsTimeUtc(XsDataPacket* thisPtr, const XsRawGpsTimeUtc* r)
{
	genericSet<XsRawGpsTimeUtc, XsRawGpsTimeUtcVariant>(thisPtr, r, XDI_GpsTimeUtc);
}
...
/*! \relates XsDataPacket
	...
	\param r The new value that should be added to the packet
*/
void XsDataPacket_setRawGnssSatInfo(XsDataPacket* thisPtr, const XsRawGnssSatInfo* r)
{
	genericSet<XsRawGnssSatInfo, XsRawGnssSatInfoVariant>(thisPtr, r, XDI_GnssSatInfo);
}
...
void XsDataPacket_setMessage(XsDataPacket* thisPtr, const XsMessage* msg)
{
	...
	while (offset < sz)
	{
		...
		if (var)
		{
			var->readFromMessage(*msg, offset+3);
			MAP.insert(id, var);
...
*/
void XsDataPacket_toMessage(XsDataPacket const * thisPtr, XsMessage* msg)
{
...
*/
XsAwindaSnapshot* XsDataPacket_awindaSnapshot(XsDataPacket const * thisPtr, XsAwindaSnapshot* returnVal)
{
	return genericGet<XsAwindaSnapshot, XsAwindaSnapshotVariant>(thisPtr, returnVal, XDI_AwindaSnapshot);
}
...
*/
int XsDataPacket_containsAwindaSnapshot(XsDataPacket const * thisPtr)
{
...
*/
void XsDataPacket_setAwindaSnapshot(XsDataPacket* thisPtr, XsAwindaSnapshot const * data, int retransmission)
{
	genericSet<XsAwindaSnapshot, XsAwindaSnapshotVariant>(thisPtr, data, XDI_AwindaSnapshot | (retransmission ? XDI_RetransmissionFlag : XDI_None));
}
...
*/
int XsDataPacket_isAwindaSnapshotARetransmission(XsDataPacket const * thisPtr)
{
	auto it = MAP.find(XDI_AwindaSnapshot);
	if (it == MAP.end())
		return false;
	return (it->second->dataId() & XDI_RetransmissionMask) == XDI_RetransmissionMask;
}
