...
public:
	...
	virtual ~Variant() {}
	virtual void readFromMessage(XsMessage const& msg, int offset) = 0;
...
public:
	...
	void readFromMessage(XsMessage const& msg, int offset) override
	{
...
template<>
inline void GenericVariant<uint64_t, 1>::readFromMessage(XsMessage const& msg, int offset)
{
	*data() = 0;
...
struct XsAwindaSnapshotVariant : public Variant
{
	...
	XsAwindaSnapshotVariant(XsDataIdentifier id, XsAwindaSnapshot const& val) : Variant(id), m_data(val) {}
	Variant* clone() const override
	{
		return new XsAwindaSnapshotVariant(dataId(), m_data);
	}

	XsAwindaSnapshot m_data;
	void readFromMessage(XsMessage const& msg, int offset) override
	{
		...
		for (int i = 0; i < 3; ++i, offset += 4)
			m_data.m_iV[i] = XsMessage_getDataLong(&msg, offset);
		for (int i = 0; i < 3; ++i, offset += 2)
			m_data.m_mag[i] = XsMessage_getDataShort(&msg, offset);
	...
	void writeToMessage(XsMessage& msg, int offset) const override
	{
		...
		for (int i = 0; i < 3; ++i, offset += 4)
			XsMessage_setDataLong(&msg, m_data.m_iV[i], offset);
		for (int i = 0; i < 3; ++i, offset += 2)
