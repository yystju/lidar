...
XSTYPES_DLL_API uint32_t XsDeviceId_deviceType(struct XsDeviceId const* thisPtr);
XSTYPES_DLL_API uint32_t XsDeviceId_deviceTypeMask(struct XsDeviceId const* thisPtr);
...
struct XsDeviceId {
	...
	inline uint32_t deviceType() const
	{
		return XsDeviceId_deviceType(this);
	}

	/*! \brief Returns the detailed device type mask of this id */
	inline uint32_t deviceTypeMask() const
	{
		return XsDeviceId_deviceTypeMask(this);
	}
